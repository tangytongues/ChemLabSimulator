...
import { Beaker, FlaskConical, TestTube, Droplet, Thermometer } from 'lucide-react';
...
interface EquipmentProps {
  ...
  onChemicalDrop?: (chemicalId: string, equipmentId: string, amount: number) => void;
}
...
export const Equipment: React.FC<EquipmentProps> = ({ 
      ...
      return (
        <div className="relative">
          ...
          <div className="w-6 h-20 bg-gradient-to-b from-transparent to-blue-100 border-2 border-blue-400 rounded-b-lg relative">
            ...
            {/* Volume markings */}
            <div className="absolute -right-8 top-2 text-xs text-gray-600">50</div>
            <div className="absolute -right-8 top-8 text-xs text-gray-600">40</div>
            <div className="absolute -right-8 top-14 text-xs text-gray-600">30</div>
          ...
          <div className="absolute inset-0 flex items-end justify-center">
            ...
            >
              ...
              <div className="relative w-full h-full overflow-hidden rounded-b-lg">
                ...
                {chemicals.some(c => c.id === 'phenol') && chemicals.some(c => c.id === 'naoh') && (
  ...
  return (
    ...
    >
      ...
      <div className={`mb-3 transition-all duration-200 relative ${
      ...
      {chemicals.length > 0 && isOnWorkbench && (
        <div className="absolute -bottom-12 left-1/2 transform -translate-x-1/2 bg-white border-2 border-gray-300 rounded-lg px-3 py-2 text-xs shadow-lg min-w-max">
          <div className="text-gray-800 font-medium">
            {chemicals.map(chemical => chemical.name.split(' ')[0]).join(' + ')}
          </div>
          <div className="text-gray-600 text-center">
            {chemicals.reduce((sum, chemical) => sum + chemical.amount, 0).toFixed(1)} mL
          </div>
          {/* Color indicator */}
          <div 
